...
for ii = 11:length(subjectdir)
   ...
   [perf, perf_perm] = mvpa_leave_out_balldesign(session, balldesign, timerange);
    ...
    if ~exist(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep 'MEG\mvpa\significance_testing\leave_design_out\'], 'dir')
        mkdir(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep 'MEG\mvpa\significance_testing\leave_design_out\'])
    end
    save(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep 'MEG\mvpa\significance_testing\leave_design_out\performance_real_data.mat'], 'perf')
    if isfield(perf_perm, 'soccer')
        perm_soccer = perf_perm.soccer;
        save(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep 'MEG\mvpa\significance_testing\leave_design_out\performance_permutation_null_distribution_soccer.mat'], 'perm_soccer')
        clear perm_soccer
    end
    if isfield(perf_perm, 'space')
        perm_space = perf_perm.space;
        save(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep 'MEG\mvpa\significance_testing\leave_design_out\performance_permutation_null_distribution_space.mat'], 'perm_space')
        clearvars perm_space
    end
    if isfield(perf_perm, 'volley')
        perm_volley = perf_perm.volley;
        save(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep 'MEG\mvpa\significance_testing\leave_design_out\performance_permutation_null_distribution_volley.mat'], 'perm_volley')        
        clearvars perm_volley
    end
...
%     cfg.repetitions = max_numel_trials*2;
    ...
    if numel(trials_trainfold_like.trial) > 4
        bootstrap_trainfold_like = fte_subaverage(cfg, trials_trainfold_like);
    else
        bootstrap_trainfold_like = trials_trainfold_like;
    end
    if numel(trials_trainfold_dislike.trial) > 4
        bootstrap_trainfold_dislike = fte_subaverage(cfg, trials_trainfold_dislike);
    else
        bootstrap_trainfold_dislike = trials_trainfold_dislike;
    end
    ...
    disp(['starting lda on real data for balldesign ' CV(kk).design]) 
    ...
    toc
    perf.(CV(kk).design).lda.auc = auc;
    perf.(CV(kk).design).lda.confusion = confusion;
    perf.(CV(kk).design).lda.accuracy = accuracy;
    perf.(CV(kk).design).lda.confusion_percentage = confusion_percentage;
    perf.(CV(kk).design).lda.f1score = f1score;
    perf.(CV(kk).design).lda.sensitivity = sensitivity;
    perf.(CV(kk).design).lda.precision_PPV = precision_PPV;
    perf.(CV(kk).design).lda.specificity_TNR = specificity_TNR;
    perf.(CV(kk).design).lda.balanced_accuracy = balanced_accuracy;
    perf.(CV(kk).design).lda.tval = tval;
    perf.(CV(kk).design).lda.dprime = dprime;
    perf.(CV(kk).design).lda.FDR = FDR;
    perf.(CV(kk).design).lda.predicted_labels = predicted_labels;
    perf.(CV(kk).design).lda.testlabels = testlabels;
    perf.(CV(kk).design).lda.dvals = dvals;
    perf.(CV(kk).design).number_of_trials.testset = numel(clabel_test_fold);  
    perf.(CV(kk).design).number_of_trials.trainingsset = numel(clabel_train_fold);
   ...
   %%  
    ...
    disp(['creating null distribution for balldesign ' CV(kk).design])
...
%     perf_perm.(balldesign{kk}).auc = auc;
    perf_perm.(CV(kk).design).accuracy = accuracy;
    perf_perm.(CV(kk).design).confusion_percentage = confusion_percentage;
    perf_perm.(CV(kk).design).confusion = confusion;
    perf_perm.(CV(kk).design).f1score = f1score;
...
%     
    perf_perm.(CV(kk).design).number_of_trials.testset = numel(clabel_test_fold);  
    perf_perm.(CV(kk).design).number_of_trials.trainingsset = numel(clabel_train_fold);
...
indx_design.soccer = [find(strcmp(balldesign_array, 'gbf')) find(strcmp(balldesign_array, 'ggf')) find(strcmp(balldesign_array, 'rwf'))];
indx_design.space = [find(strcmp(balldesign_array, 'gbs')) find(strcmp(balldesign_array, 'ggs')) find(strcmp(balldesign_array, 'rws'))];
indx_design.volley = [find(strcmp(balldesign_array, 'gbv')) find(strcmp(balldesign_array, 'ggv')) find(strcmp(balldesign_array, 'rwv'))];

design = {'soccer'; 'space'; 'volley'};

for kk = 1:length(design)
    CV(kk).testset = indx_design.(design{kk});
    CV(kk).trainingsset = 1:length(balldesign_short);
    CV(kk).trainingsset(CV(kk).testset) = [];
    CV(kk).design = design{kk};
end

for kk = 1:length(design)
    CV(kk).labels_trainingsset = labels;
    CV(kk).labels_trainingsset(CV(kk).testset) = [];
    CV(kk).labels_testset = labels(indx_design.(design{kk}));
    CV(kk).balldesign_trainingsset = balldesign_short;
    CV(kk).balldesign_trainingsset(CV(kk).testset) = [];
    CV(kk).trialnumber_likes_trainingsset = numel(find(CV(kk).labels_trainingsset==1));
    CV(kk).trialnumber_dislikes_trainingsset = numel(find(CV(kk).labels_trainingsset==2));
    CV(kk).ratio_trialnumber_likes_dislikes_trainingsset = CV(kk).trialnumber_likes_trainingsset/CV(kk).trialnumber_dislikes_trainingsset ;
end
row2delete = zeros(1,length(CV));
for kk=1:length(CV)
    if ~any(CV(kk).trialnumber_likes_trainingsset) || ~any(CV(kk).trialnumber_dislikes_trainingsset)
        row2delete(kk) = 1;
    end
end

if any(row2delete)
    indx = find(row2delete);
    CV(indx)=[];
end
