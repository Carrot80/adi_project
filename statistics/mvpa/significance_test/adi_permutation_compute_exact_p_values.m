function [] = adi_permutation_compute_p_values(subjectdir, balldesign, timerange, file)

if length(balldesign) == 1
    
    mkfigures(subjectdir, balldesign, timerange, file)
    
else

for ii = 1:length(subjectdir)

load(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep 'MEG\mvpa\significance_testing\performance_real_data.mat'], 'perf')

for kk = 1:length(balldesign)
    perf_perm.(balldesign{kk}) = load(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep ...
        'MEG\mvpa\significance_testing\performance_permutation_null_distribution_' balldesign{kk} '.mat'], ['perm_' balldesign{kk}])
end
...
acc_perf_balldesigns = zeros(length(balldesign), length(time));
for kk = 1:length(balldesign)
    acc_perf_balldesigns(kk,:) = perf.(balldesign{kk}).lda.accuracy;
end
acc_mean_balldesigns_orig_data = nanmean(acc_perf_balldesigns);

acc_perm_mean_balldesigns =  perf_perm.(balldesign{1}).(['perm_' balldesign{1}]).accuracy;
for kk = 2:length(balldesign)
   acc_perm_mean_balldesigns = (acc_perm_mean_balldesigns+perf_perm.(balldesign{kk}).(['perm_' balldesign{kk}]).accuracy)./2;
end
...
savefig([subjectdir(ii).folder filesep subjectdir(ii).name filesep 'MEG\mvpa' filesep 'significance_testing\accuracy_orig_data_uncorrected.fig'])
close 
% FDR-correction:
p(2,:) = perm_timerange;
...
xlabel('time')
ylabel('accuracy')
savefig([subjectdir(ii).folder filesep subjectdir(ii).name filesep 'MEG\mvpa' filesep 'significance_testing\accuracy_orig_data_fdr_ci.fig'])
close  
...
function [] = mkfigures(subjectdir, balldesign, timerange, file)
...
for ii = 1:length(subjectdir)
    ...
    if exist(file, 'var')
        ...
        perf_perm.(balldesign{1}) = load(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep ...
                'MEG\mvpa\significance_testing\performance_permutation_null_distribution_' balldesign{1} '.mat'], ['perm_' balldesign{1}]);
    else
        load(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name filesep 'MEG\mvpa\significance_prestim\performance_real_data.mat'], 'perf')

        data_balldesign_all_subj(ii,:) = perf.(balldesign{1}).lda.accuracy;

        perf_perm.(balldesign{1}) = load(['E:\adidas\fieldtrip_Auswertung\single_subjects\' subjectdir(ii).name '\MEG\mvpa\' file], ['perm_' balldesign{1}]);
