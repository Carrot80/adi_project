...
%         if ~exist([mainpath subjectdir(ii).name filesep filename  '_' num2str(comp(pp, 1)) '_' num2str(comp(pp, 2)) '_ms_lda.fig'], 'file')
            ...
            [perf] = mvpa_leave_out_balldesign(session, balldesign, comp(pp,:), neighbours);

            if ~exist([mainpath subjectdir(ii).name filesep 'MEG_analysis\noisereduced\1_95Hz\mvpa\searchlight\'], 'dir')
        ...
        %     perf.config.methods = 'bootstrap_pseudotrials';
            ...
            perf.component = [num2str(comp(pp,1)) '_' num2str(comp(pp, 2)) '_ms'];
        ...
        %    cfg.xlim         = [0, 0];
           ...
           savefig([mainpath subjectdir(ii).name filesep filename '_' num2str(comp(pp, 1)) '_' num2str(comp(pp, 2)) '_ms_lda.fig'])
           close 
        ...
        %    cfg.xlim         = [0, 0];
           ...
           savefig([mainpath subjectdir(ii).name filesep filename '_' num2str(comp(pp,1)) '_' num2str(comp(pp,2)) '_ms_svm.fig'])
        ...
        %    cfg.xlim         = [0, 0];
           ...
           savefig([mainpath subjectdir(ii).name filesep filename '_' num2str(comp(pp,1)) '_' num2str(comp(pp,2)) '_ms_logreg.fig'])
        ...
        %    end
            ...
            save([mainpath subjectdir(ii).name filesep filename '_' num2str(comp(pp,1)) '_' num2str(comp(pp,2)) '_ms.mat'], 'perf')
%         end
    end
        clear session
