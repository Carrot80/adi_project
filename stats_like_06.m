...
session = struct;
session.trial = [session_ggv.trial session_bgv.trial];
session.time = [session_ggv.time session_bgv.time];
session.labels = [session_ggv.labels session_bgv.labels];
session.labels(22:end)=2;

for k=1:length(session.trial)
    trials{k}=session.trial{k}(ind_cluster,:);
end
    
ind_ggv = find(session.labels==1);
ggv = [];
ggv.trial=trials(ind_ggv);
ggv.time=session.time(ind_ggv);

for k=1:length(atlas_cluster)
    label{k} = [num2str(atlas_cluster{k,3}) '_' atlas_cluster{k,2}]; 
end
ggv.label = label';
ggv.dimord = 'rpt_chan_time';

cfg=[];
cfg.keeptrials = 'yes';
ggv_avg = ft_timelockanalysis(cfg, ggv);


% zeitintervall anpassen, evtl. an rms anpassen?

ind_bgv = find(session.labels==2);
bgv = [];
bgv.trial= trials(ind_bgv);
bgv.time=session.time(ind_bgv);
bgv.label = label';
bgv.dimord = 'rpt_chan_time';

cfg=[];
cfg.keeptrials = 'yes';
bgv_avg = ft_timelockanalysis(cfg, bgv);
...
switch smaller_cond.label
    case 1
       smaller_cond.name = 'ggv';
    case 2
       smaller_cond.name = 'bgv';
end
...
switch cond_tall.label
    case 1
       cond_tall.name = 'ggv';
       ggv.trial = ggv.trial(ind_trial_perm);
       ggv.time = ggv.time(ind_trial_perm);
    case 2
       cond_tall.name = 'bgv';
       bgv.trial = bgv.trial(ind_trial_perm);
       bgv.time = bgv.time(ind_trial_perm);
end       
...
ntrials_cond = [length(session_ggv.trial) length(session_bgv.trial)];
...
cfg.ivar     = 1;
cfg.uvar     = 2;
load ('E:\adidas\fieldtrip_Auswertung\single_subjects\nl_adi_06\MEG\sourcespace\source_avg\run1\source_avg_appended_conditions_bp1_45Hz.mat', 'source_avg')
...
[stat]=ft_timelockstatistics(cfg, ggv, bgv);

save([path 'stats'], 'stat')
% 
% design = zeros(1,size(ggv.trial,1)) + size(bgv.trial,1);
% design(1,1:size(ggv.trial,1)) = 1;
% design(1,size(ggv.trial,1)+1:(size(ggv.trial,1)+size(bgv.trial,1))) = 2;
...
% avg_bgv = grandavg_bgv;
% avg_bgv = rmfield(avg_bgv, 'individual');
% avg_bgv.label = grandavg_bgv.label;
% avg_ggv = grandavg_ggv;
% avg_ggv = rmfield(avg_ggv, 'individual');
% 

bgv_avg.avg = squeeze(mean(bgv_avg.trial));
ggv_avg.avg = squeeze(mean(ggv_avg.trial));

bgv_avg.dimord = 'chan_time';
ggv_avg.dimord = 'chan_time';

time1=nearest(ggv_avg.time, time(1));
time2=nearest(ggv_avg.time, time(2));

ggv_avg_selected = ggv_avg;
ggv_avg_selected = rmfield(ggv_avg_selected, 'trial');
ggv_avg_selected.avg=ggv_avg.avg(:,time1:time2);
ggv_avg_selected.time=ggv_avg.time(1,time1:time2);

bgv_avg_selected = bgv_avg;
bgv_avg_selected = rmfield(bgv_avg_selected, 'trial');
bgv_avg_selected.avg=bgv_avg.avg(:,time1:time2);
bgv_avg_selected.time=bgv_avg.time(1,time1:time2);

cfg  = [];
cfg.operation = 'subtract';
cfg.parameter = 'avg';
raweffectggv_vs_bgv = ft_math(cfg,ggv_avg_selected,bgv_avg_selected); % geht das überhaupt im source space?
...
[i1,i2] = match_str(raweffectggv_vs_bgv.label, stat.label);
...
for k=1:length(sign_time)
    ...
    if 1 == length(unique(roi_num))
        ...
        scatter3(chanpos(ind,1),chanpos(ind,2),chanpos(ind,3), [], raweffectggv_vs_bgv.avg(ind, ind_sum_pos_neg(k)), 'filled')
...
%     set(h_legend,'Position',[0 0 0.4429 0.3349])
    ...
    if k==1 
        export_fig ([ path filesep balldesign '_sign_clusters.pdf']);
    else
        export_fig ([path filesep balldesign '_sign_clusters.pdf'], '-append')
    end
end

savefig(h, [path filesep balldesign '_sign_clusters.fig'])
close(h)
...
for i=1:length(bgv_avg)
    data_selected = ft_selectdata(cfg, bgv_avg{i});
    bgv_avg_selected{i} = data_selected;
    clear data_selected
end

for i=1:length(ggv_avg)
    data_selected = ft_selectdata(cfg, ggv_avg{i})
    ggv_avg_selected{i} = data_selected;
    clear data_selected
end
