...
%  legend(Name)
 ...
 close 
    ...
    figure
    [ph, msg] = jbfill(time, std_Accuracy_pos, std_Accuracy_neg, [0.1, 0.5, 0.5], [0.1, 0.1, 0.1],1, 0.2)
    hold on
    plot(time, mean_Accuracy, 'k', 'LineWidth', 1) 
    hold on
    [ph, msg] = jbfill(time, max_Accuracy, std_Accuracy_pos, 'magenta', [0.1, 0.1, 0.1],1, 0.2)
    hold on
    [ph, msg] = jbfill(time, std_Accuracy_neg, min_Accuracy, 'magenta', [0.1, 0.1, 0.1],1, 0.2)
    grid on
    ylim([0.2 1])
    xlabel('time');
