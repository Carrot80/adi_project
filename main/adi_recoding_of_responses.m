function [] = adi_recoding_of_responses(path2data, subject_list, ball_combinations2recode, triggercodes, triggercodepattern, run)


for i = 1:length(subject_list)
    
    fileList_Export_Bst2FT = dir(fullfile([path2data subject_list{i} '\MEG_EEG_input\noisereduced\1_95Hz\02_Export_Bst2FT\'], ['*ike500_' run '.mat'])); 
    dontcare_new_trials = [];
    for k=1:length(fileList_Export_Bst2FT)
        
        load([path2data subject_list{i} filesep 'MEG_EEG_input\noisereduced\1_95Hz\02_Export_Bst2FT\' fileList_Export_Bst2FT(k).name], 'RetVal')
        path_cleanMEG = [path2data subject_list{i} filesep 'MEG_analysis\noisereduced\1_95Hz\01_clean\'];
        load([path_cleanMEG 'alt\' fileList_Export_Bst2FT(k).name], 'cleanMEG')
        
        for j=1:length(ball_combinations2recode.(subject_list{i}))
            ind = char(triggercodes(find(strcmp(ball_combinations2recode.(subject_list{i})(j), char(triggercodepattern)) == true)));
            triggercodes2recode{j} = ind;
        end

        adi_recode(RetVal, cleanMEG, triggercodes2recode, run, path_cleanMEG, fileList_Export_Bst2FT(k).name, subject_list{i}, triggercodes, triggercodepattern);
...
function adi_recode(RetVal, cleanMEG, triggercodes2recode, run, path_cleanMEG, condition, subject, triggercodes, triggercode_labels)
