...
%run 1:
condition1run1 = ([inPath, condition1, '500_1_', freqbandname, '.mat']);
condition2run1 = ([inPath, condition2, '500_1_', freqbandname, '.mat']);
% hier weitermachen:
if ~exist ([outPath, NameCond1vs2, '500_1_', freqbandname, '.mat'], 'file') && exist(condition1run1, 'file') && exist(condition2run1, 'file')
    [data_bpfreq_condition1] = adi_bpfilter_EEG(inPath, outPath, freqbandname, condition1, run1)
    [data_bpfreq_condition2] = adi_bpfilter_EEG(inPath, outPath, freqbandname, condition2, run1)
    [Condition1vs2] = adi_crossvalidation_EEG (condition1run1, condition2run1, freqbandname, NameCond1vs2, latency, '500_1', outPath)
