...
%   save ([outPath 'MEG\sourcespace\runs_appended\virtsens\' cfg_virtsens '_' condition '_allRuns_', freqname, '.mat'], 'vs_allRuns', '-v7.3');
 ...
 for k = 1:length(freqname)
     fileName = [outPath_extdisc '\virtsens\virtsens_all_conditions_allRuns_', freqname{k}, '.mat'];
...
%     save ([outPath_extdisc 'virtsens_cond_concatenated\virtsens_' freqname{k} '.mat'], 'session') 
...
data(:,end+1)=sessions.labels;
data2=data(:,[1:1457,end]);

rng(1)
split_training_testing = cvpartition(session.labels, 'KFold', 10);
err = zeros(split_training_testing.NumTestSets,1);
for i = 1:split_training_testing.NumTestSets
    trIdx = split_training_testing.training(i);
    teIdx = split_training_testing.test(i);
    ytest = classify(data(teIdx,:),data(trIdx,:),session.labels(trIdx));
    err(i) = sum(~strcmp(ytest,session.labels(teIdx)));
end
% create a training set:
training_set = split_training_testing.training(data);
...
SVMModel = fitcsvm(data,Y,'Standardize',true,'KernelFunction','RBF',...
    'KernelScale','auto');

% neighorhood feature selection:
% featuresel = fscnca(data(:, end-1), data(:, end), 'Lambda' ,0.005, 'Verbose', 1);
featuresel2 = fscnca(data(:, 1:end-1), data(:, end),  'Standardize', true, 'Verbose', 1);
featuresel = fscnca(data2(:, 1:end-1), data2(:, end),  'Standardize', true, 'Verbose', 1);


% select features with weight above1:
selected_feature_indx2 = find(featuresel2.FeatureWeights > 0.1);  
data_selected = data(:, selected_feature_indx2);
data_selected(:, end+1)=data(:,end);
% plot feature weights:
stem(featuresel2.FeatureWeights, 'bo');
%load saved array of selected feature indices to ensure reproducability:
load('SelectedFeatures.mat')    

data_fds = fileDatastore(fullfile('D:\Kirsten\virtsens_adi_06\virtsens_cond_concatenated\'),'ReadFcn',@load,'FileExtensions','.mat');
data_dir = fullfile([pwd '\'], 'virtsens*.mat'); 
folderlist = dir(data_dir);
