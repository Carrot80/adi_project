function adi_statistics_sensor (inPath, outPath, freqbandname, condition1, condition2, latency)

time =  latency(1,:);
NameCond1vs2 = strcat(condition1, '_vs_', condition2);

%run 1:
condition1run1 = strcat(inPath, condition1, '500_1.mat');
condition2run1 = strcat(inPath, condition2, '500_1.mat');
...
%run3:
condition1run3 = strcat(inPath, condition1, '500_3.mat');
condition2run3 = strcat(inPath, condition2, '500_3.mat');
if ~exist (strcat(outPath, NameCond1vs2, '_500_3', freqbandname, '.mat'), 'file') && exist(condition1run3, 'file') && exist(condition2run3, 'file')
...
cfg.channel     = 'MEG';
cfg.vartrllength = 2;
tCondition1     = ft_timelockanalysis(cfg, condition1run.cleanMEG_interp);    
tCondition2     = ft_timelockanalysis(cfg, condition2run.cleanMEG_interp); 
...
cfg.design  = [ones(size(condition1run.cleanMEG_interp.trial,2),1); 2*ones(size(condition2run.cleanMEG_interp.trial,2),1)]';
cfg.resample = 'true';
