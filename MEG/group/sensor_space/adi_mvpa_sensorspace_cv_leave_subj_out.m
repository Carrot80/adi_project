...
for cc = 1:length(CV.pairs)
    X_test_like = [like.trial ]
    
    
%     balance

    % Get default hyperparameters for the logreg classifier
    for tt = 1:length(session.time{1})
        cf{tt} = train_lda(param, X, clabel);
