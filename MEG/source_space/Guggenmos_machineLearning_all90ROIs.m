...
% Parameters
n_perm = 10;  % number of permutations, default = 20
...
% else
    ...
    fn_outPath = [outPath 'MEG\sourcespace\90ROIs_plus_freqbands\allruns\Guggenmos_decoding_results\'];
    if ~exist (fn_outPath, 'dir')
        mkdir (fn_outPath)
    end
    save([fn_outPath session.meanmax '_5_freqbands'], 'result')
% end
...
if 3 == length(conditions) 
    figure; axis tight
    hold on
    plot(session.time, 100*squeeze(result.svm(1,2,:))+50, 'b-', 'linewidth', 1)
    plot(session.time, 100*squeeze(result.svm(1,3,:))+50, 'r-', 'linewidth', 0.5)
    plot(session.time, 100*squeeze(result.svm(2,3,:))+50, 'k-', 'linewidth',0.5)
    plot([-0.5 1], [50 50], 'k-')
    xlim([-0.5 1])
    xlabel('Time [s]')
    ylabel('Classification accuracy svm')
    legend('like vs dislike', 'like vs dontcare', 'dislike vs dontcare')
    savefig([fn_outPath session.meanmax '_5_freqbands_svm.fig'])
    close
    figure; axis tight
    hold on
    plot(session.time, 100*squeeze(result.weird(1,2,:))+50, 'b-', 'linewidth', 1)
    plot(session.time, 100*squeeze(result.weird(1,3,:))+50, 'r-', 'linewidth', 0.5)
    plot(session.time, 100*squeeze(result.weird(2,3,:))+50, 'k-', 'linewidth', 0.5)
    plot([-0.5 1], [50 50], 'k-')
    xlim([-0.5 1])
    xlabel('Time [s]')
    ylabel('Classification accuracy weird')
    legend('like vs dislike', 'like vs dontcare', 'dislike vs dontcare')
    savefig([fn_outPath session.meanmax '_5_freqbands_weird.fig'])
    close
    figure; axis tight
    hold on
