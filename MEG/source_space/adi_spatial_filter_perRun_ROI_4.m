...
mri_realigned_cm = ft_convert_units(mri_realigned, 'cm');% ensure no unit mismatch
norm=ft_volumenormalise([],mri_realigned_cm);

cfg = []; % hochauflösendes grid wird benötigt: hier weitermachen
% cfg.grid.xgrid  = -20:0.1:20; % 
% cfg.grid.ygrid  = -20:0.1:20;
% cfg.grid.zgrid  = -20:0.1:20;
cfg.grid.unit   = 'cm';
cfg.grid.tight  = 'yes';
cfg.grid.dim = mri_realigned_cm.dim;
cfg.inwardshift = -1.5;
cfg.headmodel        = vol; % template volume
template_grid  = ft_prepare_sourcemodel(cfg);
cfg = [];
cfg.parameter = 'all';
template_grid_interp = ft_sourceinterpolate(cfg, template_grid, norm)
template_grid_interp=ft_convert_units(template_grid_interp, 'cm')


pos_ind_roi=template_grid_interp.inside(indx_roi,:);% xyz positions in mni coordinates
