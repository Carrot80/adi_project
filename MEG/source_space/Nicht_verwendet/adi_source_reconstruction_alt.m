function [] = adi_source_reconstruction(path2vol, path2data, mriPath, outPath, run, freq, condition1, condition2, condition3)

load ([path2data, condition1 '500_' run '_' freq '.mat'])
% grad = ft_convert_units(data_bpfreq.grad, 'cm');
% data_bpfreq.grad = grad;

% % prepare individual leadfield:
% cfg                 = [];
% cfg.grad            = grad;
% cfg.headmodel       = vol_indiv;
% cfg.reducerank      = 2;
% % cfg.channel         = {'MEG'};
% % cfg.normalize = 'yes';
% cfg.grid.resolution = 1;   % use a 3-D grid with a 1 cm resolution
% cfg.grid.unit       = 'cm';
% [grid_indiv] = ft_prepare_leadfield(cfg);

% figure;
% ft_plot_mesh(grid_indiv.pos(grid_indiv.inside,:));
% hold on
% ft_plot_vol(vol_indiv,  'facecolor', 'cortex', 'edgecolor', 'none');alpha 0.5; camlight;
...
cfg = [];
cfg.covariance = 'yes';
cfg.covariancewindow = [-.5 .5];
cfg.keeptrials = 'yes';
tdata_bpfreq = ft_timelockanalysis(cfg, data_bpfreq);
 
cfg = [];
cfg.covariance='yes';
cfg.keeptrials = 'yes';
tdata_pre = ft_timelockanalysis(cfg, data_pre);
tdata_post = ft_timelockanalysis(cfg, data_post);

% cfg = [];
% cfg.channels = 'MEG';
% cfg.keeptrials = 'yes';
% cfg.covariance = 'yes';
% cfg.covariancwindow = 'all';
% cfg.vartrllength = 2;      
% tdataAll = ft_timelockanalysis(cfg, dataAll)
% dataAll.cov = tdataAll.cov;
% tdata_pre = ft_timelockanalysis(cfg, data_pre)
% tdata_post = ft_timelockanalysis(cfg, data_post)
...
source_tdata_bpfreq = ft_sourceanalysis(cfg, tdata_bpfreq);

% Subsequently we reconstruct the activity in the pre and post stimulus intervals using the precomputed filters.
cfg = [];
cfg.method = 'lcmv';
cfg.grid = grid;
cfg.grid.filter = source_tdata_bpfreq.avg.filter;
cfg.vol = hdm_ind;
cfg.rawtrial = 'yes';
source_tdatapre = ft_sourceanalysis(cfg, tdata_pre);
source_tdatapost = ft_sourceanalysis(cfg, tdata_post);
...
ntrials = numel(source_tdatapre.trial);
...
cfg.ivar     = 1;
cfg.uvar     = 2;
stat = ft_sourcestatistics(cfg,source_tdatapre,source_tdatapost);
