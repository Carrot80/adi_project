...
subjects_dir([1],:) = [];
list_favorite_balls([1],:) = [];

fsample = 256;
sample_steps = 1/fsample;
time = -0.5:sample_steps:1;
samples_beg = nearest(time, 0.560);
samples_end = nearest(time, 0.74);
...
%     for kk = 1:length(perf.crossval_balldesign)
%         num_of_trials.(subjects_dir(ii).name).(perf.crossval_balldesign(kk).design) = perf.number_of_trials{kk};    
%     end

end

for ii = 1:size(perf_all,2)
...
for kk = 1:size(perf_unpreferred_gbv,2)
    mean_unpreferred_gbv(kk) = mean(perf_unpreferred_gbv(ind_true_gbv,kk));
end
...
for kk = 1:size(perf_unpreferred_gbs,2)
    mean_unpreferred_gbs(kk) = mean(perf_unpreferred_gbs(ind_true_gbs,kk));
end
...
for kk = 1:size(perf_unpreferred_ggs,2)
    mean_unpreferred_ggs(kk) = mean(perf_unpreferred_ggs(ind_true_ggs,kk));
end
...
for kk = 1:size(perf_unpreferred_rws,2)
    mean_unpreferred_rws(kk) = mean(perf_unpreferred_rws(ind_true_rws,kk));
end
...
for kk = 1:size(perf_unpreferred_gbf,2)
    mean_unpreferred_gbf(kk) = mean(perf_unpreferred_gbf(ind_true_gbf,kk));
end
...
for kk = 1:size(perf_unpreferred_ggf,2)
    mean_unpreferred_ggf(kk) = mean(perf_unpreferred_ggf(ind_true_ggf,kk));
end
...
for kk = 1:size(perf_unpreferred_ggv,2)
    mean_unpreferred_ggv(kk) = mean(perf_unpreferred_ggv(ind_true_ggv,kk));
end

% perf_unpreferred_ggv = perf_unpreferred_ggv(ind_true_ggv,:);

figure; hold on;
plot(time, mean_unpreferred_ggv, 'b')
plot(time, mean_unpreferred_ggf, 'b')


figure; hold on;
plot(time, weighted_mean_Fav1, 'k')
plot(time, Fav_2_4, 'b')
plot(time, mean_unpreferred_gbs, 'g')
plot(time, mean_unpreferred_ggs, 'r')
plot(time, mean_unpreferred_gbv, 'y')
plot(time, mean_unpreferred_rws, '--')
...
figure; hold on;
plot(time, nanmean(perf_unpreferred_rws))
